...
const authPublicAPIs = require('../../middleware/authPublicAPIs')
...
const {
  ...
  getResetToken,
  sendMessage
} = require('./utils/nodemailerMessageHandler')
...
//@desc     Register user
//@access   Public, but authPublicAPI secret required
...
router.delete('/', auth, async (req, res) => {
  try {
    const response = await User.findByIdAndRemove(req.user.id).select(
      '-password'
    )
